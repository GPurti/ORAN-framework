...
#include "../../../src/sm/prova_sm/prova_sm_agent.h"
#include "../../../src/sm/prova_sm/prova_sm_ric.h"
...
{
  assert(read != NULL);
  assert(read->type == PROVA_STATS_V0);

  fill_prova_ind_data(&read->prova_stats);
  cp.hdr = cp_prova_ind_hdr(&read->prova_stats.hdr);
  cp.msg = cp_prova_ind_msg(&read->prova_stats.msg);
}
...
void check_indication(sm_agent_t* ag, sm_ric_t* ric)
{
 ...
 sm_ag_if_rd_t msg = ric->proc.on_indication(ric, &sm_data);
  ...
  assert(msg.type == PROVA_STATS_V0);


  prova_ind_data_t* data = &msg.prova_stats;

 if(msg.prova_stats.msg.rb != NULL){
      assert(msg.prova_stats.msg.len != 0);
 } 


  assert(eq_prova_ind_hdr(&data->hdr, &cp.hdr) == true);
  assert(eq_prova_ind_msg(&data->msg, &cp.msg) == true);
  assert(eq_prova_call_proc_id(data->proc_id, cp.proc_id) == true);

  free_prova_ind_hdr(&data->hdr);
  free_prova_ind_msg(&data->msg);
...
int main()
{
  ...
  sm_agent_t* sm_ag = make_prova_sm_agent(io_ag);

  sm_ric_t* sm_ric = make_prova_sm_ric();
