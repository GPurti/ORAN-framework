...
{
  assert(read != NULL);
  assert(read->type == NEW_STATS_V0);

  fill_new_ind_data(&read->new_stats);
  cp.hdr = cp_new_ind_hdr(&read->new_stats.hdr);
  cp.msg = cp_new_ind_msg(&read->new_stats.msg);
}
...
void check_indication(sm_agent_t* ag, sm_ric_t* ric)
{
 ...
 if(msg.new_stats.msg.ngut != NULL){
