...
{
  assert(next_pow2(len) >= sizeof(new_ind_msg_t) - sizeof(new_ng_u_tunnel_stats_t*) && "Less bytes than the case where there are not active ngu Tunnel! Next pow2 trick used for aligned struct");
  new_ind_msg_t ret = {0};
