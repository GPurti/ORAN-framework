...
byte_array_t new_enc_ind_hdr_plain(new_ind_hdr_t const* ind_hdr)
{
  ...
  ba.buf = malloc(sizeof(new_ind_msg_t));
  assert(ba.buf != NULL && "memory exhausted");
  memcpy(ba.buf, ind_hdr, sizeof(new_ind_hdr_t));
...
byte_array_t new_enc_ctrl_hdr_plain(new_ctrl_hdr_t const* ctrl_hdr)
{
  ...
  memcpy(ba.buf, ctrl_hdr, sizeof(new_ctrl_hdr_t));
...
byte_array_t new_enc_ctrl_msg_plain(new_ctrl_msg_t const* ctrl_msg)
{
  ...
  byte_array_t  ba = {0};
  ba.buf = malloc(sizeof(new_ctrl_msg_t)); 
  assert(ba.buf != NULL && "Memory exhausted");

  memcpy(ba.buf, ctrl_msg, sizeof(new_ctrl_msg_t));
