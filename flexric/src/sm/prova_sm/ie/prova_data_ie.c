...
prova_ind_msg_t cp_prova_ind_msg(prova_ind_msg_t const* src)
{
  ...
  prova_ind_msg_t cp = {.len = src->len, .tstamp = src->tstamp}; 

  if(cp.len > 0){
    cp.rb = calloc(cp.len, sizeof(prova_radio_bearer_stats_t));
    assert(cp.rb != NULL && "memory exhausted");
  }

  memcpy(cp.rb, src->rb, sizeof(prova_radio_bearer_stats_t)*cp.len);
...
bool eq_prova_ind_msg(prova_ind_msg_t* m0, prova_ind_msg_t* m1)
{
 ...
 prova_radio_bearer_stats_t const* rb0 = &m0->rb[i];
 prova_radio_bearer_stats_t const* rb1 = &m1->rb[i];
 if(
...
void free_prova_ind_data(prova_ind_data_t* ind)
{
  ...
  free_prova_call_proc_id(ind->proc_id); 
}

prova_ind_data_t cp_prova_ind_data(prova_ind_data_t const* src)
{
  assert(src != NULL);
  prova_ind_data_t dst = {0};

  dst.hdr = cp_prova_ind_hdr(&src->hdr);
  dst.msg = cp_prova_ind_msg(&src->msg);

  if(src->proc_id != NULL){
    dst.proc_id = malloc(sizeof(prova_call_proc_id_t));
    assert(dst.proc_id != NULL && "Memory exhausted");
    *dst.proc_id = cp_prova_call_proc_id(src->proc_id);
  }
