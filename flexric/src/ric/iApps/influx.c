...
void notify_influx_listener(sm_ag_if_rd_t const* data)
{
  ...
  assert(data->type == MAC_STATS_V0 || data->type == RLC_STATS_V0 || data->type == PDCP_STATS_V0 || data->type == SLICE_STATS_V0 || data->type == KPM_STATS_V0 || data->type == GTP_STATS_V0 || data->type == NEW_STATS_V0);
...
//  printf("Influx db data called!!!\n");
  if(data->type == MAC_STATS_V0){
    ...
    for(uint32_t i = 0; i < new->len; ++i){
      ...
      char stats[1024] = {0};
      to_string_new_rb(&new->ngut[i], new->tstamp, stats, 1024);
