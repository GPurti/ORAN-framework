...
void create_new_bearer_table(sqlite3* db)
{
  ...
  "CREATE TABLE NEW_bearer(tstamp INT CHECK(tstamp > 0),"
                            "ngran_node INT CHECK(ngran_node >= 0 AND ngran_node < 9),"
                            "mcc INT,"
                            "mnc INT,"
                            "mnc_digit_len INT,"
...
static
int to_sql_string_new_rb(global_e2_node_id_t const* id,new_ng_u_tunnel_stats_t* new, int64_t tstamp, char* out, size_t out_len)
{
  assert(new != NULL);
  assert(out != NULL);
  const size_t max = 1024;
...
void write_new_stats(sqlite3* db, global_e2_node_id_t const* id, new_ind_data_t const* ind)
{
  ...
  for(size_t i = 0; i < ind_msg_new->len; ++i){
    pos += to_sql_string_new_rb(id, &ind_msg_new->ngut[i], ind_msg_new->tstamp, buffer + pos, 2048 - pos);
  }
...
void write_db_sqlite3(sqlite3* db, global_e2_node_id_t const* id, sm_ag_if_rd_t const* rd)
{
  assert(db != NULL);
  assert(rd != NULL);
  assert(rd->type == MAC_STATS_V0 || rd->type == RLC_STATS_V0|| rd->type == PDCP_STATS_V0 || rd->type == SLICE_STATS_V0 ||rd->type ==KPM_STATS_V0 ||rd->type == GTP_STATS_V0 || rd->type == NEW_STATS_V0);
  ...
  if(rd->type == MAC_STATS_V0){
    ...
    write_new_stats(db, id, &rd->new_stats);
  } else {
